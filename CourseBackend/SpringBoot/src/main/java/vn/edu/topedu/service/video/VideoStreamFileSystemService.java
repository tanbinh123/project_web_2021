...
public class VideoStreamFileSystemService implements IVideoStreamService {
    ...
    public ResponseEntity<byte[]> prepareContent(String fullFileName, String range) {
        long rangeStart = 0;
        long rangeEnd;
        byte[] data;
        Long fileSize;
        ...
        Matcher m = Pattern.compile("^(.+)\\.(\\w+)$").matcher(fullFileName);
		if (m.find()) {
			fileName = m.group(1);
			extend = m.group(2);
        ...
        try {
            ...
            if (range == null) {
                return ResponseEntity.status(HttpStatus.OK)
                        .header(CONTENT_TYPE, VIDEO_CONTENT + extend)
                        .header(CONTENT_LENGTH, String.valueOf(fileSize))
                        .body(readByteRange(fullFileName, rangeStart, fileSize - 1)); // Read the object and convert it as bytes
            }
            String[] ranges = range.split("-");
